...
public class FCFS 
{
	 ...
	 public FCFS(ArrayList<Process> process, int nbrCores) {
		        ...
		        for(int i=0; i<CPUList.length;i++)
		        {
		        	...
		        	if(clockCycle[i] >= ReadyList.get(readyProcessInd).getStartTime())
		        	{
		        		...
		        		if(!IOrequest.isEmpty())
		        		{
		        			...
		        			for(int j=0; j<(running.getRemainingDuration() + IOrequestTime)%running.getDuration();j++)
		        			{
		        				CPUList[i].add(running.getId());
		        			}
		        			
		        			//setting the new burst-time left of that process
		        			running.setRemainingDuration(running.getRemainingDuration()-(running.getRemainingDuration() + IOrequestTime)%running.getDuration());
		        			clockCycle[i] = CPUList[i].size();
		        			//Jump straight to the IOtime for next processes
		        			running.setStartTime(clockCycle[i] + 2);
		        			//re-add the process at the end of the list
		        			ReadyList.add(running);
		        		}
		        		else
		        		{
		        			//if no io left just add the remaining amount
		        			for(int j=0; j<running.getRemainingDuration();j++)
		        			{
		        				CPUList[i].add(running.getId());
		        			}
		        			clockCycle[i] = CPUList[i].size();
